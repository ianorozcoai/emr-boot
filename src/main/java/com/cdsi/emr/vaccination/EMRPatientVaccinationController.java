...
public class EMRPatientVaccinationController {
    ...
    public String listAll(Model model, @PathVariable long patientId) {
        ...
        Patient patient = optionalPatient.orElseGet(Patient::new);
    ...
    public String savePatientVaccination(
        ...
        if (files.length > 0 && files[0].getOriginalFilename().isEmpty()) {
            EMRPatientVaccination emrVacc = this.emrPatientVaccinationRepository.findById(emrPatientVaccination.getId())
                    .orElseGet(EMRPatientVaccination::new);
        ...
        this.emrPatientVaccinationRepository.save(emrPatientVaccination);
