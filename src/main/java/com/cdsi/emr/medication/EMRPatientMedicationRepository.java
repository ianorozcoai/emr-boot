...
public interface EMRPatientMedicationRepository extends JpaRepository<EMRPatientMedication, Long> {
    ...
    List<EMRPatientMedication> findByPatientIdOrderByDateCreatedDesc(long patientId);
    ...
    @Query(nativeQuery = true,
            value = "select * from emrpatient_medication where patient_id in ?1 order by date_created")
    List<EMRPatientMedication> findAllByPatientIdOrderByDateCreated(List<Long> ids);
}
